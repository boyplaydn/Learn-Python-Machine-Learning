...
theta = np.loadtxt("test.out")

# print(model.shape[0], 'model')
...
for rect in rects:
    ...
    number = np.concatenate((number, np.ones((number.shape[0], 1))), axis = 1)
    print(number)
    #bien roi thanh mang 1 chieu voi kich thuoc 28*28
    predict = sigmoid(np.dot(number, theta))
